//! The session: every pane and every classic tab's split tree, plus the
//! shared view state (active pane/tab, floating pane, scroll offset). Pane
//! state (parser + pty channels) lives in `pane`, spawning in `spawn`; this
//! module owns only the session invariants.
